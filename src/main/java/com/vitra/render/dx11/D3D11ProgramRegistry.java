...
/**
 * Registry for associating OpenGL program IDs with DirectX 11 pipelines.
 * Similar to VulkanMod's VkGlProgram system.
 *
 * This class maintains the mapping between Minecraft's OpenGL-style programId
 * and our DirectX 11 shader pipeline handles, ensuring that when a shader
...
public class D3D11ProgramRegistry {
    ...
    /**
     * Map of OpenGL program ID -> DirectX 11 Pipeline
     * This is the key mapping that associates Minecraft's shader system with our DirectX 11 backend.
     */
    private static final ConcurrentHashMap<Integer, DirectX11Pipeline> programToPipeline = new ConcurrentHashMap<>();

    /**
     * Register a pipeline with a program ID.
     * This should be called after creating a shader pipeline in ShaderInstanceM.
     *
     * @param programId The OpenGL program ID from ShaderInstance
     * @param pipeline The DirectX 11 pipeline to associate
     */
    public static void registerPipeline(int programId, DirectX11Pipeline pipeline) {
        if (programId <= 0) {
        ...
        DirectX11Pipeline existing = programToPipeline.put(programId, pipeline);
     ...
     * @return The associated DirectX 11 pipeline, or null if not found
     */
    public static DirectX11Pipeline getPipeline(int programId) {
        DirectX11Pipeline pipeline = programToPipeline.get(programId);
    ...
    public static void unregisterPipeline(int programId) {
        DirectX11Pipeline removed = programToPipeline.remove(programId);
