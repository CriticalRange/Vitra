 ...
 */
public class VitraRendererExposer {
    private static final Logger LOGGER = LoggerFactory.getLogger("VitraRendererExposer");
     ...
     * Simplified using BgfxUtils
     */
    public static void exposeRendererInfo() {
        try {
            LOGGER.info("=== EXPOSING BGFX RENDERER INFO TO SYSTEM ===");

            // Use consolidated utility for renderer information
            String rendererInfo = BgfxUtils.getRendererInfo();
            LOGGER.info("BGFX Renderer Info: {}", rendererInfo);

            // Update current renderer information
            int rendererType = BgfxUtils.getRendererType();
            ...
            LOGGER.info("Actual BGFX backend: {}", getRendererTypeName(rendererType));
            LOGGER.info("Reported to system: DirectX 11 (for monitoring tool compatibility)");
            exposeDirect3D11ToSystem("DirectX 11 (BGFX)");

            // Log memory statistics using utility
            LOGGER.info("Memory Statistics: {}", BgfxUtils.getMemoryStats());
    ...
    public static void updateRendererInfo() {
        if (!rendererInfoUpdated) {
            ...
            // Just refresh the current info using utility
            int currentType = BgfxUtils.getRendererType();
