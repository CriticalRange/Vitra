...
public class BgfxWindow {
    ...
    public void handleUpdateDisplay() {
        ...
        try {
            if (bgfxInitialized && VitraMod.getRenderer() != null && VitraMod.getRenderer().isFullyInitialized()) {
                // Submit BGFX frame instead of OpenGL swap
                BGFX.bgfx_frame(false);
                LOGGER.debug("BGFX frame submitted successfully");
            } else {
                LOGGER.debug("BGFX not ready, skipping frame submission");
            }
        } catch (Exception e) {
